...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$nom || !$prix || !$categorie_id) {
        ...
        if (!$error) {
            $stmt = $pdo->prepare("UPDATE produits SET categorie_id = ?, nom = ?, description = ?, prix = ?, stock = ?, image = ?, marque = ?, garantie = ? WHERE id = ?");
            $stmt->execute([$categorie_id, $nom, $description, $prix, $stock, $imagePath, $marque, $garantie, $id]);
...
$imgSrc = $produit['image'] ? $produit['image'] : '';
...
<head>
    ...
    <title>Modifier un Produit - TechStore</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: Arial, sans-serif; }
        ...
        .file-upload .preview { max-width: 200px; max-height: 200px; margin-top: 15px; border-radius: 10px; }
        .file-upload .current-img { max-width: 200px; max-height: 200px; margin-top: 10px; border-radius: 10px; }
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="prix" class="form-control" step="0.01" min="0" value="<?= htmlspecialchars($produit['prix']) ?>" required>
                    </div>

                    <div class="form-group">
                        <label>Stock</label>
                        <input type="number" name="stock" class="form-control" min="0" value="<?= htmlspecialchars($produit['stock']) ?>">
                    </div>

                    <div class="form-group">
                        <label>Marque</label>
                        <input type="text" name="marque" class="form-control" value="<?= htmlspecialchars($produit['marque'] ?? '') ?>">
                    </div>

                    <div class="form-group">
                        <label>Garantie</label>
                        <input type="text" name="garantie" class="form-control" value="<?= htmlspecialchars($produit['garantie'] ?? '') ?>" placeholder="ex: 12 mois">
                    </div>
                    ...
                    <div class="form-group full">
                        ...
                        <div class="file-upload" id="file-upload-area" onclick="document.getElementById('image-input').click()">
                            ...
                            <?php if ($imgSrc): ?>
                                <img src="<?= htmlspecialchars($imgSrc) ?>" class="current-img" id="current-image" alt="Image actuelle">
                            <?php endif; ?>
                            <img id="image-preview" class="preview" alt="Aperçu" style="display:none;">
                        </div>
                    </div>
                </div>
        ...
        function previewImage(event) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    document.querySelector('.file-upload p').textContent = 'Nouveau fichier : ' + file.name;
                };
